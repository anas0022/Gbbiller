                ...
                render: function (data, type, row) {
                    return `<button class="btn btn-info btn-circle-wrapper" 
                              data-bs-toggle="modal"
                                ...
                                onclick="editSubscription(${row.id} , '${row.method}', '${row.dis}', '${row.icon}')"
                                title="Edit Method">
...
function deleteSubscription(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const row = $(`button[onclick="deleteSubscription(${id})"]`).closest('tr');

            row.addClass('deleting');

            setTimeout(() => {
                $.ajax({
                    url: `/supperadmin/subscription/delete/${id}`,
                    method: "GET",
                    dataType: "json",
                    beforeSend: function() {
                        $('#custom-loader').show();
...
function editSubscription(id, method, dis, icon) {
   window.location.href = subedit.replace(':id', id);
